...
dirPath = os.path.abspath('.') + r'/' + uploadPath
...
def upload(request):
    if request.method == 'GET':
        ...
        uploadFiles = request.FILES.getlist('files[]')  # 获取上传文件
        for uploadFile in uploadFiles:
            ...
            while (os.path.exists(dirPath + r'/' + fileName)):
...
def change(request):
    ...
    filePath = dirPath + r'/' + changefile
    if os.path.exists(filePath):
        if getFileType(filePath) == 'doc':
            docx_path = docSaveToDocx(filePath)
            if docx_path:
                file_name = str(docx_path).split('/')
                file_name = file_name[len(file_name) - 1]
                print(docx_path)
                print(file_name)
                message = 'doc 成功转换为docx' + '  文件名称：' + file_name
            else:
                message = 'doc 转换为docx 失败'
